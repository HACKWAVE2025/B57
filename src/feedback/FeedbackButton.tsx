...
export const FeedbackButton: React.FC<FeedbackButtonProps> = ({
  ...
  const positionClasses = {
    "bottom-right": `bottom-${6 + (customOffset.y || 0)} right-${
      6 + (customOffset.x || 0)
    }`,
    "bottom-left": `bottom-${6 + (customOffset.y || 0)} left-${
      6 + (customOffset.x || 0)
    }`,
    "top-right": `top-${6 + (customOffset.y || 0)} right-${
      6 + (customOffset.x || 0)
    }`,
    "top-left": `top-${6 + (customOffset.y || 0)} left-${
      6 + (customOffset.x || 0)
    }`,
  };
    ...
    if (finalPosition === "auto" && finalAvoidInputAreas) {
      const detectInputAreas = () => {
        ...
        const chatInputs = document.querySelectorAll(
          '[data-component="ai-chat"] input, [data-component="ai-chat"] textarea'
        );
  ...
  const currentPosition =
    finalPosition === "auto" ? smartPosition : finalPosition;
  ...
  const handleMouseMove = (e: MouseEvent) => {
    ...
    const newPosition = {
      x: Math.max(
        0,
        Math.min(window.innerWidth - 60, (dragPosition?.x || 0) + deltaX)
      ),
      y: Math.max(
        0,
        Math.min(window.innerHeight - 60, (dragPosition?.y || 0) + deltaY)
      ),
    };
  ...
  return (
    <>
      ...
      <motion.div
        className={`fixed ${
          finalPosition === "draggable" ? "" : positionClasses[currentPosition]
        } z-40 ${
          isDragging
            ? "cursor-grabbing"
            : draggable && finalPosition === "draggable"
            ? "cursor-grab"
            : ""
        }`}
      ...
      >
        <div className="relative">
          ...
          <motion.button
            ...
            } bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white rounded-full shadow-lg hover:shadow-xl transition-all duration-300 flex items-center justify-center group ${
              ...
              {isHovered && finalShowLabel && (
                <motion.div
                  initial={{
                    opacity: 0,
                    scale: 0.8,
                    x: currentPosition.includes("right") ? 10 : -10,
                  }}
                  animate={{
                    opacity: 1,
                    scale: 1,
                  ...
                  className={`absolute ${
                    currentPosition.includes("right")
                      ? "right-full mr-3"
                      : "left-full ml-3"
