...
import { ATSScoreStorage } from "../../../utils/atsScoreStorage";
...
export const ATSScoreGenerator: React.FC<ATSScoreGeneratorProps> = ({
  ...
  const handleGenerateScore = useCallback(async () => {
    ...
    try {
      ...
      try {
        const scoreId = ATSScoreStorage.saveScore(result, {
          resumeText,
          jobDescriptionText: jobDescriptionText.trim() || undefined,
          resumeFileName: resumeFile?.name,
        });
        console.log("✅ Score saved to history:", scoreId);

        // Try to save to backend if authenticated
        if (ATSService.isAuthenticated()) {
          await ATSScoreStorage.saveToBackend(result, {
            resumeText,
            jobDescriptionText: jobDescriptionText.trim() || undefined,
            resumeFileName: resumeFile?.name,
          });
        ...
        console.error("Failed to save score to history:", saveError);
        // Don't show error to user - score was still generated successfully
      }
  ...
  }, [resumeText, jobDescriptionText, resumeFile]);
  ...
  const handleDownloadPDF = useCallback(async () => {
    if (!scoreResult?.scoreRunId) {
      setError(
        "PDF report is only available for scores saved to the server. Please use the backend API to generate reports with downloadable PDFs."
      );
      return;
    }
