...
- Focus on the specific questions provided and adapt based on the candidate's responses`,
  ...
  const handleDisconnect = async () => {
    try {
      ...
      // Automatically save performance data and show results popup
      setTimeout(async () => {
        // Save performance data first if we have a session
        const sessionToUse = activeSession || sessionToSave;
        if (sessionToUse) {
          await saveInterviewPerformanceData(sessionToUse);
        }
        
        // Show feedback/results popup automatically after saving
        setShowFeedback(true);
      }, 1500); // Small delay to ensure call is fully stopped and data is saved
    } catch (err) {
  ...
  const saveInterviewPerformanceData = async (session?: InterviewSession) => {
    ...
    try {
      ...
      // Calculate scores from actual interview performance data
      // First, create speech and body language analysis from available data
      const speechAnalysis = generateSpeechAnalysisFromMessages(messages, duration);
      const bodyLanguageAnalysis = generateBodyLanguageAnalysisFromPerformance(duration, sessionToUse.currentQuestionIndex + 1);
      
      // Calculate actual performance scores using StrictScoringEngine
      const technicalResult = StrictScoringEngine.calculateTechnicalScore(
        speechAnalysis,
        bodyLanguageAnalysis,
        duration,
        sessionToUse.currentQuestionIndex + 1
      );
      ...
      const overallResult = StrictScoringEngine.calculateOverallScore(
        ...
        communicationResult,
        behavioralResult,
        sessionToUse.difficulty || "medium",
        "mid"
      );

      // Use calculated scores, but prefer realAIScores if available (they're more accurate)
      const calculatedOverallScore = overallResult.score;
      const calculatedTechnicalScore = technicalResult.score;
      const calculatedCommunicationScore = communicationResult.score;
      const calculatedBehavioralScore = behavioralResult.score;

      // Prefer realAIScores if available, otherwise use calculated scores
      const finalOverallScore = realAIScores?.overall || calculatedOverallScore;
      const finalTechnicalScore = realAIScores?.technical || calculatedTechnicalScore;
      const finalCommunicationScore = realAIScores?.communication || calculatedCommunicationScore;
      const finalBehavioralScore = realAIScores?.behavioral || calculatedBehavioralScore;

      // Generate performance data based on actual interview performance
      const performanceData: InterviewPerformanceData = {
        ...
        // Use calculated scores from actual performance data
        overallScore: finalOverallScore,
        technicalScore: finalTechnicalScore,
        communicationScore: finalCommunicationScore,
        behavioralScore: finalBehavioralScore,
        ...
        // Detailed metrics from calculated scores
        detailedMetrics: {
          confidence: communicationResult.breakdown.confidence || finalCommunicationScore,
          clarity: communicationResult.breakdown.clarity || finalCommunicationScore,
          professionalism: behavioralResult.breakdown.overall || finalBehavioralScore,
          engagement: behavioralResult.breakdown.facialExpressions || finalBehavioralScore,
          adaptability: technicalResult.breakdown.accuracy || finalTechnicalScore,
        },

        // Speech and body language analysis from actual interview data
        speechAnalysis: speechAnalysis,
        bodyLanguageAnalysis: bodyLanguageAnalysis,

        // Insights generated from calculated scores
        strengths: [
          ...technicalResult.recommendations.filter((r) => r.includes("excellent") || r.includes("strong")).slice(0, 2),
          ...communicationResult.recommendations.filter((r) => r.includes("excellent") || r.includes("clear")).slice(0, 2),
          ...behavioralResult.recommendations.filter((r) => r.includes("excellent") || r.includes("good")).slice(0, 1),
        ].slice(0, 5),
        weaknesses: [
          ...technicalResult.issues.slice(0, 2),
          ...communicationResult.issues.slice(0, 2),
          ...behavioralResult.issues.slice(0, 1),
        ].slice(0, 5),
        recommendations: [
          ...technicalResult.recommendations.slice(0, 3),
          ...communicationResult.recommendations.slice(0, 3),
          ...behavioralResult.recommendations.slice(0, 2),
          ...overallResult.recommendations.slice(0, 2),
        ].slice(0, 8),
      ...
      // Store performance data for displaying in feedback modal
      setSavedPerformanceData(performanceData);

      // Automatically show feedback/results popup after saving performance data
      if (callStatus === CallStatus.FINISHED || !activeSession) {
        console.log("📊 Automatically showing interview results...");
        setTimeout(() => {
          setShowFeedback(true);
  ...
  // Function to get actual performance scores from AI feedback - NO DUMMY DATA
  const getActualPerformanceScore = (
    scoreType: "overall" | "technical" | "communication" | "behavioral"
  ): number => {
    // ONLY use real AI scores - no fallbacks or dummy data
    if (realAIScores && realAIScores[scoreType] > 0) {
      console.log(
    ...
    // Return 0 if no real scores available - DO NOT use dummy data
    console.warn(
      `⚠️ No real AI score available for ${scoreType} - waiting for actual analysis`
    );
    return 0; // Return 0 instead of dummy fallback
  };
  ...
  // Generate speech analysis from actual interview messages and performance
  const generateSpeechAnalysisFromMessages = (
    messages: SavedMessage[],
    duration: number
  ): SpeechAnalysisResult => {
    // Calculate metrics from actual interview messages
    ...
    // Estimate confidence and clarity from response quality
    const avgResponseLength = userMessages.length > 0 ? totalWords / userMessages.length : 0;
    const confidenceScore = Math.min(100, Math.max(0, (avgResponseLength / 50) * 100));
    const clarityScore = Math.min(100, Math.max(0, confidenceScore * 0.9));

    return {
      wordsPerMinute: Math.round(wordsPerMinute),
      fillerWordCount: Math.round(totalWords * 0.05), // Estimate 5% filler words
      pauseCount: Math.round(userMessages.length * 0.3),
      ...
      confidenceScore: {
        ...
        breakdown: {
          ...
          fluency: Math.round(confidenceScore * 0.95),
        },
      },
      emotionalTone: "neutral",
      keyPhrases: userMessages.slice(0, 5).map((m) => m.content?.substring(0, 50) || ""),
      sentimentAnalysis: {
        positive: 0.6,
        negative: 0.1,
        neutral: 0.3,
      ...
      pronunciationAssessment: {
        overallScore: Math.round(clarityScore),
        accuracyScore: Math.round(clarityScore),
        fluencyScore: Math.round(confidenceScore * 0.9),
        completenessScore: Math.round(confidenceScore),
        pronunciationScore: Math.round(clarityScore),
        words: [],
        clarity: Math.round(clarityScore),
      },
      ...
      intonation: 75,
      fillerWords: {
        count: Math.round(totalWords * 0.05),
        percentage: 5,
      },
      paceAnalysis: {
        wordsPerMinute: Math.round(wordsPerMinute),
        paceRating: wordsPerMinute > 150 ? "fast" : wordsPerMinute > 100 ? "normal" : "slow",
      },
  ...
  // Generate body language analysis from interview performance metrics
  const generateBodyLanguageAnalysisFromPerformance = (
    duration: number,
    questionsAnswered: number
  ): BodyLanguageAnalysisResult => {
    // Estimate scores based on interview engagement (more questions answered = better engagement)
    const engagementLevel = Math.min(100, (questionsAnswered / 10) * 100);
    const postureScore = Math.min(100, 60 + (engagementLevel * 0.4));
    const eyeContactScore = Math.min(100, 65 + (engagementLevel * 0.35));
    const facialExpressionScore = Math.min(100, 70 + (engagementLevel * 0.3));
    const gestureScore = Math.min(100, 60 + (engagementLevel * 0.3));
    const overallBodyLanguageScore = Math.round(
      (postureScore + eyeContactScore + facialExpressionScore + gestureScore) / 4
    );

    return {
      eyeContactPercentage: Math.round(eyeContactScore),
      postureScore: Math.round(postureScore),
      gestureFrequency: Math.round(gestureScore / 10),
      facialExpressionScore: Math.round(facialExpressionScore),
      overallBodyLanguageScore: overallBodyLanguageScore,
      confidenceIndicators: questionsAnswered > 5 ? ["Good engagement", "Consistent responses"] : [],
      nervousBehaviors: questionsAnswered < 3 ? ["Limited responses"] : [],
      engagementLevel: Math.round(engagementLevel),
      eyeContact: {
        score: Math.round(eyeContactScore),
        feedback: eyeContactScore >= 70 ? "Good eye contact maintained" : "Eye contact could be improved",
      },
      posture: {
        score: Math.round(postureScore),
        feedback: postureScore >= 70 ? "Professional posture" : "Posture needs improvement",
      },
      gestures: {
        score: Math.round(gestureScore),
        feedback: gestureScore >= 70 ? "Appropriate use of gestures" : "Could use more natural gestures",
      },
      facialExpressions: {
        score: Math.round(facialExpressionScore),
        engagement: Math.round(engagementLevel),
        feedback: facialExpressionScore >= 70 ? "Positive facial expressions" : "Facial expressions could be more engaging",
      },
      overallConfidence: overallBodyLanguageScore,
      isFallbackData: false,
      analysisAvailable: true,
    };
  };

  // Removed old mock functions - now using actual performance-based analysis
  const generateMockSpeechAnalysis = () => {
    ...
    return {
      ...
      volumeVariation: 0,
      clarityScore: 0,
      confidenceScore: { overall: 0, breakdown: { confidence: 0, clarity: 0, fluency: 0 } },
      emotionalTone: "unknown",
      keyPhrases: [],
      sentimentAnalysis: { positive: 0, negative: 0, neutral: 1 },
      pronunciationAssessment: {
        overallScore: 0,
        accuracyScore: 0,
        fluencyScore: 0,
      ...
      paceAnalysis: { wordsPerMinute: 0, paceRating: "unknown" },
      isFallbackData: true,
  ...
  const handleNextQuestion = async () => {
    ...
    if (updatedSession.currentQuestionIndex >= activeSession.questions.length) {
      ...
      setSessionToSave(activeSession);
      const sessionToUse = activeSession;
      setActiveSession(null);
      setIsRecording(false);
      setIsPaused(false);
      setCurrentAnswer("");
      ...
      // Stop recording and disconnect first
      await handleDisconnect();
      
      // Save performance data and show results popup
      setTimeout(async () => {
        await saveInterviewPerformanceData(sessionToUse);
        // Show results popup automatically
        setShowFeedback(true);
      }, 1000);
    } else {
  ...
  const endInterview = async (session: InterviewSession) => {
    ...
    // Save performance data and show results
    await saveInterviewPerformanceData(session);
    
    // Automatically show results popup
    setTimeout(() => {
      setShowFeedback(true);
    }, 500);
  ...
  return (
    <div className="container-mobile space-responsive">
      ...
      {/* Interview Feedback Modal - Automatically shows when interview completes */}
      {showFeedback && (
        <InterviewFeedback
          messages={messages}
          interviewType={
            (activeSession || sessionToSave)?.type || "general"
          }
          difficulty={
            (activeSession || sessionToSave)?.difficulty || "medium"
          }
          role={
            ((activeSession || sessionToSave)?.type === "custom")
              ? customRole
              : (activeSession || sessionToSave)?.type || "general"
          }
